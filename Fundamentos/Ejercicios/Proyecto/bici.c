...
void bitacora(int accion,int var);
...
void Alta_Bici(TipoEstacion **InicioEst, int *AcumNumB)
{
  int Est, Espacios, Bicis,accion=201;
  TipoEstacion *Busca;
  TipoBici *NuevaBici;
  if(*InicioEst!=NULL)
    {
...
void AltaUsuario(TipoUs **Inicio, int *ultusu){
  ...
  system("clear");
  printf("El usuario fue dado de alta correctamente con el número: %i\n\n",temp->NumUs);
  bitacora(accion,temp->NumUs);
}
  ...
  if(temp==NULL)//si no se encuentra el usuario, no hace nada.
    ...
    if(temp==*Inicio){//si esta al comienzo cambia el inicio a la siguiente estructura.
      ...
      *Inicio=temp2;
      free(temp);
      system("clear");
      printf("El usuario con número %d fue dado de baja correctamente.\n\n",us);
    }
...
//Función que realiza los ajustes de renta
{
  ...
  printf("\nUsuario %d: Por favor toma la bicicleta %d de la estación %d. La bicicleta ha sido rentada correctamente.\n\n",NumUs,NumB,Est);
}
...
void Dejar_Bici(TipoEstacion *InicioEst, TipoUs *InicioUs, int NumUs, int EstD, int NumB)
{
  ...
  if(diff>1800){
    printf("\nRentaste tu bicicleta %s.\nLa regresaste %s.\nEso equivale a una multa de 500 pesos.\n",BuscaBici->TimeS,cadena);
    (BuscaUs->multa)++;
  }
...
void bitacora(int accion,int var1){
  time_t crudo;
  struct tm *tipotiempo;
...
void MostrarEst(TipoEstacion *BuscaEst){
  ...
  temp=BuscaEst;
  while(temp!=NULL){
    printf("\nExiste la estacion %s ubicada en %s.\n",temp->Nombre,temp->Calle_y_Num);
    temp=temp->ptrsig;
  }
}
